...
def validate_data_consistency(csv_handler, parquet_handler, sample_size=1000):
    ...
    if csv_data.shape == parquet_data.shape:
        logger.info(f"   ✓ 数据形状相同: {csv_data.shape}")
    else:
        results['passed'] = False
        results['errors'].append(f"数据形状不同: CSV={csv_data.shape}, Parquet={parquet_data.shape}")
        logger.error(f"   ✗ 数据形状不同: CSV={csv_data.shape}, Parquet={parquet_data.shape}")
    ...
    if csv_cols == parquet_cols:
        logger.info(f"   ✓ 列名相同: {len(csv_cols)} 列")
    else:
        ...
        if missing_in_parquet:
            ...
            logger.warning(f"   ⚠ Parquet缺少列: {missing_in_parquet}")
        if extra_in_parquet:
            results['warnings'].append(f"Parquet多出列: {extra_in_parquet}")
            logger.warning(f"   ⚠ Parquet多出列: {extra_in_parquet}")
    ...
    if csv_dates == parquet_dates:
        logger.info(f"   ✓ 日期范围相同: {len(csv_dates)} 个交易日")
        logger.info(f"     {csv_dates[0]} 至 {csv_dates[-1]}")
    else:
        results['errors'].append(f"日期范围不同")
        logger.error(f"   ✗ 日期范围不同")
        logger.error(f"     CSV: {len(csv_dates)} 天 ({csv_dates[0]} 至 {csv_dates[-1]})")
        logger.error(f"     Parquet: {len(parquet_dates)} 天 ({parquet_dates[0]} 至 {parquet_dates[-1]})")
        ...
        for col in numeric_cols:
            ...
            if pd.isna(csv_val) and pd.isna(parquet_val):
                ...
                if diff_count <= 3:  # 只显示前3个错误
                    logger.error(f"   ✗ 行 {idx}, 列 {col}: CSV={csv_val}, Parquet={parquet_val}")
                continue
            ...
            try:
                ...
                if diff > 1e-6:  # 允许微小的浮点数误差
                    ...
                    if diff_count <= 3:
                        logger.error(f"   ✗ 行 {idx}, 列 {col}: CSV={csv_val}, Parquet={parquet_val}, 差异={diff}")
    ...
    if all_match:
        logger.info(f"   ✓ 所有样本数据值匹配")
    else:
        logger.warning(f"   ⚠ 发现 {diff_count} 处数据差异")
        if max_diff > 0:
            logger.warning(f"   ⚠ 最大差异: {max_diff}")
    ...
    if not dtype_diffs:
        logger.info(f"   ✓ 数据类型一致")
    else:
        results['warnings'].append(f"数据类型差异: {len(dtype_diffs)} 列")
        logger.warning(f"   ⚠ {len(dtype_diffs)} 列数据类型不同:")
    ...
    if results['passed']:
        logger.info("✓ 数据验证通过！CSV 和 Parquet 数据一致")
    else:
        logger.error("✗ 数据验证失败！发现重大差异")
...
def benchmark_loading(data_path, use_parquet=True, num_workers=4):
    ...
    logger.info(f"测试 {format_name} 格式加载性能")
    logger.info(f"{'='*60}")
    ...
    logger.info(f"\n{format_name} 格式加载结果:")
    logger.info(f"  加载时间: {elapsed_time:.2f} 秒")
    logger.info(f"  股票数量: {info['stock_count']}")
    logger.info(f"  总数据点: {info['total_data_points']:,}")
    logger.info(f"  吞吐量: {info['total_data_points']/elapsed_time:,.0f} 行/秒")
...
if __name__ == '__main__':
    ...
    if csv_time and parquet_time:
        ...
        logger.info(f"CSV加载时间:    {csv_time:.2f} 秒")
        logger.info(f"Parquet加载时间: {parquet_time:.2f} 秒")
        logger.info(f"速度提升:       {speedup:.1f}x")
        logger.info(f"时间节省:       {((1 - 1/speedup) * 100):.1f}%")
    elif parquet_time:
        logger.info(f"Parquet加载时间: {parquet_time:.2f} 秒")
        logger.info("\n提示: CSV目录不存在，无法对比")
        logger.info(f"如需对比，请确保CSV目录存在: {args.csv_dir}")
    else:
        logger.info("\n提示: Parquet目录不存在")
        logger.info(f"请先运行: python scripts/convert_to_parquet.py --data-dir {args.csv_dir}")
